    ...
    <nav className={`navbar ${scrolled ? 'scrolled' : ''}`}>
      <div className="nav-inner">
        ...
        <ul className={`nav-links ${open ? 'open' : ''}`}>
          {navLinks.map(link => (
            <li key={link.label}>
              <a href={link.href} className={location.pathname === link.href ? 'active' : ''} onClick={() => handleClick(link.href)}>
                {link.label}
              </a>
            </li>
          ))}
        ...
        <button className={`burger ${open ? 'open' : ''}`} onClick={() => setOpen(!open)} aria-label="Meniu">
          <span></span>
          <span></span>
          <span></span>
        </button>
